    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-neutral-800 p-4 bg-neutral-900 space-y-3">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          ...
          <input
            ...
            value={form.cb}
            onChange={(e) => setForm({ ...form, cb: e.target.value })}
          />
          <input
            className="px-3 py-2 rounded-lg bg-neutral-800 border border-neutral-700"
      ...
      <div className="grid gap-3">
        {positions.map((p) => (
          <div
            key={p.id}
            className="rounded-xl border border-neutral-800 p-4 bg-neutral-900 flex flex-col md:flex-row md:items-center md:justify-between gap-3"
          >
            <div className="space-y-1">
              <div className="font-medium">{p.symbol}</div>
              <div className="text-sm text-neutral-400">
                Qty {p.qty} · Cost {p.cost_basis}
              </div>
              <div className="text-sm text-neutral-400">
                Px {p.current_price ?? '-'} · MV {p.market_value ?? '-'} · P/L{' '}
                {p.unrealized_pl ?? '-'} ({p.pl_pct?.toFixed(2) ?? '-'}%)
              </div>
            </div>
      ...
      {summary && (
        <div className="rounded-xl border border-neutral-800 p-4 bg-neutral-900">
          ...
          <div className="text-sm text-neutral-300">
            Cost: {summary.total_cost} · Value: {summary.total_value} · P/L: {summary.total_pl} (
