...
class J6NotificationIntegrator:
    ...
    async def on_user_followed(self, follower_user_data: Dict[str, Any], followed_user_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Follow notification triggered: {follower_user_data.get('username')} -> {followed_user_data.get('username')}")
    ...
    async def on_dm_message_received(
        ...
        try:
            ...
            await notification_integration.on_dm_message_sent(sender_user_data, recipient_user_data, message_data)
            self._integration_stats["dm_notifications"] += 1
            
            logger.info(f"DM notification triggered: {sender_user_data.get('username')} -> {recipient_user_data.get('username')}")
    ...
    async def on_user_mentioned(
        ...
        try:
            ...
            logger.info(f"Mention notification triggered: @{mentioned_user_data.get('username')} by {mentioning_user_data.get('username')}")
