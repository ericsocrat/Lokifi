...
def _compute_fields(p: PortfolioPosition) -> Dict[str, Any]:
    ...
    pl_pct = ((cur - p.cost_basis) / p.cost_basis * 100.0) if (cur is not None and p.cost_basis) else None
    return dict(
        current_price=cur,
...
async def _maybe_create_alerts(owner: str, symbol: str, cost_basis: float):
    ...
    try:
        ...
        a1 = AlertModel(
            id=__import__("uuid").uuid4().hex, type="price_threshold",
            symbol=symbol, timeframe=user_tf, active=True,
            created_at=__import__("time").time(), min_interval_sec=600,
...
def list_positions(request: Request, handle: Optional[str] = Query(None), authorization: Optional[str] = Header(None)):
    ...
    with get_session() as db:
        ...
        rows = db.execute(select(PortfolioPosition).where(PortfolioPosition.user_id == u.id)).scalars().all()
        out: list[PositionOut] = []
        for r in rows:
            comp = _compute_fields(r)
            out.append(PositionOut(
                id=r.id, symbol=r.symbol, qty=r.qty, cost_basis=r.cost_basis,
                tags=_tags_to_list(r.tags), created_at=r.created_at.isoformat(),
                updated_at=r.updated_at.isoformat(), **comp
            ))
        return out
...
async def add_or_update_position(
    ...
    with get_session() as db:
        ...
        if existing:
            ...
            p = PortfolioPosition(
                user_id=u.id, symbol=payload.symbol, qty=payload.qty,
                cost_basis=payload.cost_basis, tags=_tags_to_str(payload.tags),
    ...
    return PositionOut(
        id=p.id, symbol=p.symbol, qty=p.qty, cost_basis=p.cost_basis,
        tags=_tags_to_list(p.tags), created_at=p.created_at.isoformat(),
        updated_at=p.updated_at.isoformat(), **comp
    )
...
def delete_position(position_id: int, handle: Optional[str] = Query(None), authorization: Optional[str] = Header(None)):
    ...
    with get_session() as db:
        u = _user_by_handle(db, me)
        row = db.execute(
            select(PortfolioPosition).where(PortfolioPosition.id == position_id, PortfolioPosition.user_id == u.id)
        ).scalar_one_or_none()
        if not row:
