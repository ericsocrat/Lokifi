...
class AnalyticsQueryBuilder:
    ...
    def notification_analytics(self, session: Session, days_back: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_count > 0:
                ...
                analytics["overall_metrics"]["avg_delivery_rate"] = total_delivery_rate / total_count
    ...
    def user_engagement_metrics(self, session: Session, days_back: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in result:
                engagement_rate = (row.read_notifications + row.clicked_notifications) / (row.total_notifications * 2) if row.total_notifications > 0 else 0
    ...
    def _fallback_notification_analytics(self, session: Session, days_back: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "by_priority": {"normal": {"count": result, "read_rate": 0.5, "delivery_rate": 0.8}},
                "overall_metrics": {
...
class CompatibilityTester:
    ...
    def test_basic_queries(self, session: Session) -> Dict[str, bool]:
        ...
        try:
            if self.dialect == DatabaseDialect.POSTGRESQL:
                result = session.execute(text("SELECT DATE_TRUNC('day', CURRENT_TIMESTAMP)")).scalar()
            else:
        ...
        try:
            if self.dialect == DatabaseDialect.POSTGRESQL:
                result = session.execute(text("SELECT '{\"test\": \"value\"}'::jsonb ->> 'test'")).scalar()
            else:
                result = session.execute(text("SELECT JSON_EXTRACT('{\"test\": \"value\"}', '$.test')")).scalar()
        ...
        try:
            result = session.execute(text("SELECT ROW_NUMBER() OVER (ORDER BY 1) FROM (SELECT 1) t")).scalar()
    ...
    def get_compatibility_report(self, session: Session) -> Dict[str, Any]:
        ...
        if not capabilities["json_support"]:
            recommendations.append("Consider enabling JSON1 extension for SQLite or using JSON columns in PostgreSQL")
        
        if not capabilities["window_functions"]:
            recommendations.append("Window functions not available - some analytics queries will use fallbacks")
