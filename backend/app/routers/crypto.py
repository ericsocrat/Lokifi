...
async def get_crypto_prices(
    symbols: str = Query(
        ...,
        description="Comma-separated list of crypto symbols (e.g., bitcoin,ethereum,cardano)",
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=e.response.status_code, detail=f"CoinGecko API error: {e}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch crypto prices: {str(e)}"
        )
...
async def get_trending_crypto(
    limit: int = Query(
        10, ge=1, le=50, description="Number of trending cryptos to return"
    )
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch trending cryptos: {str(e)}"
        )
...
async def get_crypto_market_chart(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch market chart: {str(e)}"
        )
