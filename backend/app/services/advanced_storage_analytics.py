...
class AdvancedStorageAnalytics:
        ...
        try:
            async for session in get_db_session():
                ...
                if metrics.total_messages > 0:
                    ...
                    total_content_length = await session.scalar(
                        select(func.sum(func.length(AIMessage.content)))
                    ) or 0
                    
                    metrics.avg_message_size_kb = (total_content_length / metrics.total_messages) / 1024
                ...
                messages_last_24h = await session.scalar(
                    select(func.count(AIMessage.id))
                    .where(AIMessage.created_at >= yesterday)
                ) or 0
                metrics.daily_growth_rate = messages_last_24h
                
                # Weekly growth
                last_week = now - timedelta(days=7)
                messages_last_week = await session.scalar(
                    select(func.count(AIMessage.id))
                    .where(AIMessage.created_at >= last_week)
                ) or 0
                metrics.weekly_growth_rate = messages_last_week / 7
                
                # Monthly growth
                last_month = now - timedelta(days=30)
                messages_last_month = await session.scalar(
                    select(func.count(AIMessage.id))
                    .where(AIMessage.created_at >= last_month)
                ) or 0
                ...
                logger.info(f"Advanced metrics: {metrics.total_messages:,} messages, "
    ...
    async def benchmark_database_performance(self) -> List[PerformanceBenchmark]:
        ...
        operations = [
            ("message_insert", "INSERT INTO ai_messages (thread_id, role, content, created_at) VALUES (1, 'user', 'test', NOW())"),
    ...
    async def generate_storage_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "executive_summary": {
                ...
                "predicted_30_day_growth": round(metrics.predicted_size_30_days - metrics.total_size_mb, 2)
            },
        ...
        logger.info(f"📊 Storage report generated: {metrics.total_messages:,} messages, "
