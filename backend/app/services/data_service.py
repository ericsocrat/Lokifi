...
class SymbolDirectory:
    ...
    async def search_symbols(
        self, query: str, asset_type: AssetType | None = None, limit: int = 50
    ) -> list[Symbol]:
        ...
        for symbol in self.symbols.values():
            ...
            if query in symbol.symbol or query.lower() in symbol.name.lower():
...
class OHLCAggregator:
        ...
        for provider_config in self.providers:
            ...
            try:
                ...
                if data:
                    ...
                    logger.info(
                        f"Successfully fetched {len(data)} bars for {symbol} from {provider_config.provider.value}"
                    )
                    return data

            except Exception as e:
                logger.warning(
                    f"Provider {provider_config.provider.value} failed for {symbol}: {e}"
                )
                continue
    ...
    async def _fetch_yahoo_finance(
        self, config: DataProviderConfig, symbol: str, timeframe: str, limit: int
    ) -> list[OHLCData]:
                ...
                ohlc_data.append(
                    OHLCData(
                        ...
                        timestamp=datetime.strptime(
                            date_str,
                            "%Y-%m-%d %H:%M:%S" if " " in date_str else "%Y-%m-%d",
                        ),
