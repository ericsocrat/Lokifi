...
class AIContextManager:
    ...
    async def get_conversation_context(
        ...
        with self.session_factory() as db:
            ...
            ).order_by(desc(AIMessage.created_at)).limit(max_messages).all()
            ...
            for msg in recent_messages:
                ...
                provider_messages.append(AIProviderMessage(
                    role=role,
                    content=msg.content
                ))
            
            # Check if we need to summarize older context
            total_message_count = db.query(AIMessage).filter(
                AIMessage.thread_id == thread_id
            ).count()
    ...
    async def analyze_conversation_style(
        ...
        with self.session_factory() as db:
            user_messages = db.query(AIMessage.content).filter(
                AIMessage.thread_id == thread_id,
                AIMessage.role == "user"
            ).all()
    ...
    async def create_context_summary(
        ...
        try:
            ...
            if provider:
                ...
                summary_messages = [AIProviderMessage(
    ...
    async def _get_or_create_context_summary(
        ...
        if thread_id in self.context_cache:
            ...
            if memory.last_updated > datetime.utcnow() - timedelta(hours=1):
                return memory.context_summary
        
        # Get older messages (excluding recent ones)
        older_messages = db.query(AIMessage).filter(
            AIMessage.thread_id == thread_id
        ).order_by(AIMessage.created_at).offset(recent_message_limit).all()
    ...
    async def get_user_context_across_threads(
        ...
        with self.session_factory() as db:
            ...
            recent_threads = db.query(AIThread).filter(
                AIThread.user_id == user_id
            ).order_by(desc(AIThread.updated_at)).limit(limit).all()
            ...
            return {
                ...
                "dominant_communication_style": style_counter.most_common(1)[0][0] if style_counter else "neutral",
