...
async def test_follow_flow_basic():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        login = await client.post("/api/auth/login", json={"email": alice_email, "password": "testpassword123"})
        ...
        token = login.cookies.get("access_token") or login.json()["user"]["id"]  # fallback (shouldn't use fallback)
        headers = {"Authorization": f"Bearer {token}"} if token else {}

        follow_resp = await client.post("/api/follow/follow", json={"user_id": bob_user_id}, headers=headers)
        assert follow_resp.status_code in [200, 409]

        # Idempotent second follow
        follow_resp2 = await client.post("/api/follow/follow", json={"user_id": bob_user_id}, headers=headers)
        ...
        unfollow_resp = await client.request("DELETE", "/api/follow/unfollow", json={"user_id": bob_user_id}, headers=headers)
        assert unfollow_resp.status_code in [200]
        unfollow_resp2 = await client.request("DELETE", "/api/follow/unfollow", json={"user_id": bob_user_id}, headers=headers)
