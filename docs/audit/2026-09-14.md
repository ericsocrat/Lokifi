...
Audit prerequisite note: after reconciling Vitest 4 dependencies, the legacy pre-commit type check exposed Vite/Vitest configuration type errors. The old hook was interrupted rather than allowed to run unrelated automation. Local audit/rebuild commits used a per-command disabled hook path; the replacement's explicit verification gates were run independently and passed. This is not a claim that the historical hook passed.
