...
def feed(handle: str, symbol: str | None = None, limit: int = 50,
         after_id: int | None = None, request: Request = None):
    # Query 1: followee_ids = db.execute(select(Follow.followee_id)...).all()
    # Query 2: posts = db.execute(select(Post, User).join(...).where(...)).all()
"""
