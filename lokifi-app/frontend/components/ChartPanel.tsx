...
export default function ChartPanel() {
  ...
  useEffect(() => {
    ...
    if (inds.stddev) {
      ...
      overlayLine(sd.mid, 1);
      overlayLine(sd.upper, 1);
      overlayLine(sd.lower, 1);
    }
    ...
    if ((inds.rsi || inds.macd) && subRef.current) {
      ...
      const sync = () => {
        const rScale = subChart.timeScale();
        const mScale = chart.timeScale();
        rScale.subscribeVisibleTimeRangeChange((range: { from: number; to: number }) => {
          if (range) mScale.setVisibleRange(range);
        });
        mScale.subscribeVisibleTimeRangeChange((range: { from: number; to: number }) => {
          if (range) rScale.setVisibleRange(range);
        });
      };
      sync();
    }
    ...
    const nearestSnap = (t: number, p: number) => {
      ...
      for (let i = 0; i < cs.length; i++) {
        const d = Math.abs(cs[i].ts / 1000 - t);
        if (d < bestd) {
          bestd = d;
          best = i;
        }
      ...
      let bestp = prices[0],
        bestpd = Math.abs(prices[0] - p);
      for (const pr of prices) {
        const d = Math.abs(pr - p);
        if (d < bestpd) {
          bestpd = d;
          bestp = pr;
        }
  ...
  useEffect(() => {
    const canvas = overlayRef.current;
    if (!canvas || !data || !ref.current) return;
    const el = ref.current as HTMLDivElement;
    const width = el.clientWidth;
    const height = inds.rsi || inds.macd ? 360 : 520;
    ...
    const dist = (x1: number, y1: number, x2: number, y2: number, px: number, py: number) => {
      ...
      const t = Math.max(0, Math.min(1, dot / len_sq));
      const xx = x1 + t * C,
        yy = y1 + t * D;
    ...
    const drawLine = (x1: number, y1: number, x2: number, y2: number, dash = false) => {
      ctx.save();
    ...
    const drawLabel = (x: number, y: number, text: string) => {
      ...
      ctx.fillText(text, rx + padX, ry + h / 2 + 4);
      ctx.restore();
    };
    ...
    const drawInfo = (
      ...
      const dp = p2 - p1;
      const pct = p1 ? (dp / p1) * 100 : 0;
      const midx = (x1 + x2) / 2,
        midy = (y1 + y2) / 2;
      drawLabel(
        midx - 36,
        midy + 16,
        `${ang.toFixed(1)}°  |  Δ${fmt(dp)}  (${pct >= 0 ? '+' : ''}${pct.toFixed(2)}%)  |  ${bars.toFixed(1)} bars`
      );
    };
    ...
    function lineParams(l: any) {
      const x1 = timeToX2(l.a.t),
        y1 = priceToY2(l.a.p);
      const x2 = timeToX2(l.b.t),
        y2 = priceToY2(l.b.p);
      return { x1, y1, x2, y2 };
    }
    function intersect(l1: any, l2: any) {
      const { x1, y1, x2, y2 } = lineParams(l1);
      const { x1: x3, y1: y3, x2: x4, y2: y4 } = lineParams(l2);
      ...
      function within(l: any) {
        const { x1, y1, x2, y2 } = lineParams(l);
    ...
    if (mq) {
      ...
      ctx.setLineDash([5, 4]);
      ctx.strokeRect(
        Math.min(mq.x0, mq.x1),
        Math.min(mq.y0, mq.y1),
        Math.abs(mq.x1 - mq.x0),
        Math.abs(mq.y1 - mq.y0)
      );
      ctx.restore();
    }
    ...
    if (hud) {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      (ctx as any).roundRect?.(bx, by, 160, 56, 10);
      if (!(ctx as any).roundRect) {
        ctx.rect(bx, by, 160, 56);
      }
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = '#e5e7eb';
    ...
    if (hov?.type === 'intersection') {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      (ctx as any).roundRect?.(tx, ty, w, h, 8);
      if (!(ctx as any).roundRect) {
        ctx.rect(tx, ty, w, h);
      }
      ctx.fill();
    ...
    if (ghost) {
      ...
      const sh: any = ghost;
      if (sh.type === 'fib') {
        const x1 = timeToX(sh.a.t);
        const y1 = priceToY(sh.a.p);
        const x2 = timeToX(sh.b.t);
        const y2 = priceToY(sh.b.p);
        const levels = (sh as any).levels ?? [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1];
        const high = Math.max(y1, y2);
        const low = Math.min(y1, y2);
        for (const lv of levels) {
          const y = high - (high - low) * lv;
          drawLine(Math.min(x1, x2), y, Math.max(x1, x2), y, true);
        }
        drawLabel(x1 + 6, y1 - 14, `$${fmt(sh.a.p)}`);
        drawLabel(x2 + 6, y2 - 14, `$${fmt(sh.b.p)}`);
        if (selectedIds.has(sh.id)) {
          drawHandle(x1, y1);
          drawHandle(x2, y2);
        }
      }
    ...
    function shapeIntersectsRect(
      ...
      if (sh.type === 'hline') {
        const y = priceToY((sh as any).y);
        return y >= top && y <= bottom;
      }
      const a: any = (sh as any).a,
        b: any = (sh as any).b;
      ...
      return (
        ...
        ptInRect(x1, y1, left, top, right, bottom) ||
        ptInRect(x2, y2, left, top, right, bottom)
      );
    }
    ...
    function hitTest(clientX: number, clientY: number) {
      ...
      for (const sh of drawStore.get().shapes) {
        if (
          ...
          const x1 = timeToX((sh as any).a.t),
            y1 = priceToY((sh as any).a.p);
          const x2 = timeToX((sh as any).b.t),
            y2 = priceToY((sh as any).b.p);
          ...
          const y0 = priceToY((sh as any).y);
          if (Math.abs(x - 20) <= 6 && Math.abs(y - y0) <= 6) return { id: sh.id, handle: 'y' };
        } else if (sh.type === 'channel3') {
          const x1 = timeToX((sh as any).a.t),
            y1 = priceToY((sh as any).a.p);
          const x2 = timeToX((sh as any).b.t),
            y2 = priceToY((sh as any).b.p);
          const cx = timeToX((sh as any).c.t),
            cy = priceToY((sh as any).c.p);
          ...
          if (Math.abs(x - right) <= 6 && Math.abs(y - top) <= 6)
            return { id: sh.id, handle: 'tr' };
          if (Math.abs(x - left) <= 6 && Math.abs(y - bottom) <= 6)
            return { id: sh.id, handle: 'bl' };
          if (Math.abs(x - right) <= 6 && Math.abs(y - bottom) <= 6)
            return { id: sh.id, handle: 'br' };
        }
      }
      ...
      for (const sh of drawStore.get().shapes) {
        if (sh.type === 'trendline' || sh.type === 'ray') {
          ...
          const minx = Math.min(x1, x2) - 6,
            ...
            miny = Math.min(y1, y2) - 6,
            maxy = Math.max(y1, y2) + 6;
          if (x >= minx && x <= maxx && y >= miny && y <= maxy) return { id: sh.id, handle: null };
        } else if (sh.type === 'hline') {
          const y0 = priceToY((sh as any).y);
          if (Math.abs(y - y0) <= 6) return { id: sh.id, handle: null };
          ...
          if ((sh as any).width != null) {
            const midP = ((sh as any).a.p + (sh as any).b.p) / 2;
            const yMid = priceToY(midP);
            const yUp = priceToY(midP + (sh as any).width);
            pix = Math.abs(yUp - yMid);
          } else {
            const c = (sh as any).c;
            const tt1 = (sh as any).a.t,
              tt2 = (sh as any).b.t;
            const priceAtT = (t: number) => {
              if (tt1 === tt2) return (sh as any).a.p;
              const frac = (t - tt1) / (tt2 - tt1);
              return (sh as any).a.p + frac * ((sh as any).b.p - (sh as any).a.p);
            };
            const widthPrice = Math.abs(c.p - priceAtT(c.t));
            const midP = ((sh as any).a.p + (sh as any).b.p) / 2;
            const yMid = priceToY(midP);
            const yUp = priceToY(midP + widthPrice);
            pix = Math.abs(yUp - yMid);
          }
          const offx = nx * pix,
            offy = ny * pix;
          ...
          const inside = (function () {
            ...
            const vpx = x - mx,
              vpy = y - my;
            const along = vpx * ux + vpy * uy;
            const norm = vpx * nx + vpy * ny;
            return Math.abs(norm) < pix && along > -len * 0.1 && along < len * 1.1;
          })();
          ...
          if (x >= left && x <= right && y >= top && y <= bottom)
            return { id: (sh as any).id, handle: null };
        }
      }
    ...
    function onDown(e: PointerEvent) {
      ...
      const t0 = timeFromX(x);
      const p0 = priceFromY(y);
      if (drawStore.get().tool === 'cursor') {
        ...
        if (e.shiftKey) {
          ...
          drawStore.selectOne(id);
        }
        if (handle) {
          dragMode = { kind: 'handle', id, handle, start: nearestSnap(t0, p0) };
          return;
        }
        const sel = drawStore.get().selectedIds;
        dragMode = {
          kind: sel.includes(id) && sel.length > 1 ? 'move-group' : 'move',
          id,
          start: nearestSnap(t0, p0),
        };
        return;
      }
      const { t, p } = nearestSnap(t0, p0);
      if (drawStore.get().tool === 'hline') {
        drawStore.addShape({ id: crypto.randomUUID(), type: 'hline', y: p } as any);
        return;
      }
      ...
      if (dragMode) {
        ...
        if (dragMode.kind === 'handle' && dragMode.id) {
          ...
          drawStore.updateShape(dragMode.id, (s: any) => {
            ...
            if ('a' in s && 'b' in s) {
              if (which === 'a') return { ...(s as any), a: { t, p } };
              if (which === 'b') return { ...(s as any), b: { t, p } };
            }
            return s as any;
          });
          ...
          dragMode.start = { t, p };
        } else if (dragMode.kind === 'move-group') {
          drawStore.moveSelectedBy(dt, dp);
          dragMode.start = { t, p };
        }
        return;
      }
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (!e.ctrlKey && !e.metaKey) {
        if (e.key === 'v' || e.key === 'V') {
          pluginManager.setActiveTool(null);
          drawStore.setTool('cursor');
        }
        if (e.key === 't' || e.key === 'T') {
          pluginManager.setActiveTool(null);
        ...
        if (e.key === 'r' || e.key === 'R') {
          pluginManager.setActiveTool('ruler-measure');
        }
        if (e.key === 'c' || e.key === 'C') {
          if (e.shiftKey) pluginManager.setActiveTool('parallel-channel-3pt');
          else pluginManager.setActiveTool('parallel-channel');
        }
        if (e.key === 'f' || e.key === 'F') {
          pluginManager.setActiveTool('fib-extended');
        }
      }
      ...
      if (
        ...
        (document.activeElement as any)?.tagName !== 'INPUT'
      ) {
        e.preventDefault();
