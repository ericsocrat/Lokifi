...
export const drawStore = {
  ...
  setTool(tool: Tool) { _state = { ..._state, tool }; listeners.forEach((l: any) => l(_state)); },
  setSnap(snap: boolean) { _state = { ..._state, snap }; listeners.forEach((l: any) => l(_state)); },

  setSelection(ids: string[]) { _state = { ..._state, selectedIds: Array.from(new Set(ids)) }; listeners.forEach((l: any) => l(_state)); },
  selectOne(id: string | null) { _state = { ..._state, selectedIds: id ? [id] : [] }; listeners.forEach((l: any) => l(_state)); },
  toggle(id: string) { const s = new Set(_state.selectedIds); if (s.has(id)) s.delete(id); else s.add(id); _state = { ..._state, selectedIds: Array.from(s) }; listeners.forEach((l: any) => l(_state)); },
  clearSelection() { _state = { ..._state, selectedIds: [] }; listeners.forEach((l: any) => l(_state)); },
  ...
  clear() { const sym = symbolStore.get(); const tf = timeframeStore.get(); undoStack.push(_state.shapes); _state = { ..._state, shapes: [], selectedIds: [] }; save(sym, tf, []); listeners.forEach((l: any) => l(_state)); },
  undo() { if (!undoStack.length) return; redoStack.push(_state.shapes); const prev = undoStack.pop()!; _state = { ..._state, shapes: prev }; const sym = symbolStore.get(); const tf = timeframeStore.get(); save(sym, tf, _state.shapes); listeners.forEach((l: any) => l(_state)); },
  redo() { if (!redoStack.length) return; undoStack.push(_state.shapes); const next = redoStack.pop()!; _state = { ..._state, shapes: next }; const sym = symbolStore.get(); const tf = timeframeStore.get(); save(sym, tf, _state.shapes); listeners.forEach((l: any) => l(_state)); },
  removeSelected() { const ids = new Set(_state.selectedIds); if (!ids.size) return; const sym = symbolStore.get(); const tf = timeframeStore.get(); undoStack.push(_state.shapes.map((s: any) => ({ ...(s as any) }))); redoStack = []; _state = { ..._state, shapes: _state.shapes.filter((s: any) => !ids.has(s.id)), selectedIds: [] }; save(sym, tf, _state.shapes); listeners.forEach((l: any) => l(_state)); },
  loadCurrent() { const sym = symbolStore.get(); const tf = timeframeStore.get(); _state = { ..._state, shapes: load(sym, tf) }; listeners.forEach((l: any) => l(_state)); },
};
