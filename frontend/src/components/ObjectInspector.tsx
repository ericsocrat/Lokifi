...
export default function ObjectInspector() {
  ...
  const mixed = (key: any, pick:(d:any)=>any) => {
    ...
    return drawings.some((d: any) => pick(d) !== v) ? key : v
  }
    ...
    <div className='rounded-2xl border border-white/15 p-3 space-y-3'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-2'>
          <button className='px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10'
                  onClick={()=>s.toggleLockSelected()}>{drawings.every((d: any) =>d.locked) ? 'Unlock' : 'Lock'}</button>
          <button className='px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10'
                  onClick={()=>s.toggleVisibilitySelected()}>{drawings.every((d: any) =>d.hidden) ? 'Show' : 'Hide'}</button>
        </div>
      </div>
      ...
      <div className='space-y-2'>
        ...
        <input className='w-full bg-transparent border border-white/15 rounded px-2 py-1 text-sm'
               value={typeof mixed('—', d=>(d as any).name||'') === 'string' ? ((first as any)?.name||'') : '—'}
               onChange={(e: any) =>s.renameSelected(e.target.value)} placeholder='Untitled object'/>
      </div>
      ...
      <div className='grid grid-cols-2 gap-2 text-sm'>
        ...
        <div>
          ...
          <input type='number' min={1} max={6} className='w-full bg-transparent border border-white/15 rounded px-2 py-1'
                 value={(first as any)?.style?.strokeWidth ?? 2}
                 onChange={(e: any) =>s.setSelectedStyle({ strokeWidth: Math.max(1, parseInt(e.target.value||'1',10)) })}/>
        </div>
        ...
        <div>
          ...
          <select className='w-full bg-transparent border border-white/15 rounded px-2 py-1'
                  value={(first as any)?.style?.dash || 'solid'}
                  onChange={(e: any) =>s.setSelectedStyle({ dash: e.target.value as any })}>
        ...
        <div>
          ...
          <input type='range' min={20} max={100}
                 value={Math.round(((first as any)?.style?.opacity ?? 1)*100)}
                 onChange={(e: any) =>s.setSelectedStyle({ opacity: parseInt(e.target.value,10)/100 })}/>
        </div>

        <div className='col-span-2'>
          <div className='text-xs opacity-70 mb-1'>Fill (rect/ellipse/channel)</div>
          <input type='color' className='w-full h-8 bg-transparent border border-white/15 rounded'
                 value={((first as any)?.style?.fill as any) || '#000000'}
                 onChange={(e: any) =>s.setSelectedStyle({ fill: e.target.value })}/>
        </div>
      </div>

      {drawings.every((d: any) =>d.kind==='fib') && (
        <FibEditor />
      )}
...
function FibEditor() {
  ...
  const drawings = useChartStore(st => st.drawings.filter((d: any) => st.selection.has(d.id)))
  const first = drawings[0]
  const levels = ((first as any)?.fibLevels ?? s.drawingSettings.fibDefaultLevels).slice().sort((a: number, b: number) => a - b)
...
function parseLevels(src: string): number[] {
  return src.split(',').map((s: any) =>parseFloat(s.trim())).filter((n: any) =>!Number.isNaN(n) && n>=0 && n<=1)
}
