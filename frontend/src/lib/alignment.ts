...
export function align(drawings: Drawing[], ids: Set<string>, dir: 'left'|'right'|'top'|'bottom') {
  const selected = drawings.filter((d: any) =>ids.has(d.id))
  if (selected.length < 2) return drawings
  const boxes = new Map<string,B>(selected.map((d: any) =>[d.id, bbox(d)]))
  let target = 0
  switch (dir) {
    case 'left':   target = Math.min(...selected.map((d: any) =>boxes.get(d.id)!.minX)); break
    case 'right':  target = Math.max(...selected.map((d: any) =>boxes.get(d.id)!.maxX)); break
    case 'top':    target = Math.min(...selected.map((d: any) =>boxes.get(d.id)!.minY)); break
    case 'bottom': target = Math.max(...selected.map((d: any) =>boxes.get(d.id)!.maxY)); break
  }
  const out = drawings.map((d: any) =>{
...
export function distribute(drawings: Drawing[], ids: Set<string>, axis: 'h'|'v') {
  ...
  const boxes = selected.map((d: any) =>({ id:d.id, ...bbox(d) }))
  if (axis==='h') {
    boxes.sort((a: any, b: any) =>a.minX-b.minX)
    ...
    return drawings.map((d: any) => out.has(d.id) ? ({ ...d, points: d.points.map((p: any) =>({ x:p.x + (out.get(d.id) || 0), y:p.y })) }) : d)
  } else {
    boxes.sort((a: any, b: any) =>a.minY-b.minY)
    ...
    return drawings.map((d: any) => out.has(d.id) ? ({ ...d, points: d.points.map((p: any) =>({ x:p.x, y:p.y + (out.get(d.id) || 0) })) }) : d)
  }
}
