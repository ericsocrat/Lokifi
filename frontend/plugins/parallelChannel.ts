...
export const parallelChannel: ToolPlugin = {
  ...
  kind: "tool",
  onPointerDown(e, ctx){
    const g = (globalThis as any)[ghostKey]; if (g && g.type==='channel' && g._plugin==='parallel-channel') (globalThis as any)[ghostKey] = null;
    const { snapped } = ctx.xy(e);
    const st: any = (parallelChannel as any)._state || ((parallelChannel as any)._state = { a: null as null | { t:number, p:number } });
  ...
  onPointerMove(e, ctx){ const st: any = (parallelChannel as any)._state; if (!st?.a) return false; const { snapped } = ctx.xy(e); const a = st.a, b = snapped; const s = pluginSettingsStore.get(); let preview: any = { type:'channel', a, b, width: Math.abs(a.p)*(s.channelDefaultWidthPct/100) }; if (s.channelWidthMode==='pixels'){ preview.widthMode='pixels'; preview.widthPx = 20; } (globalThis as any)[ghostKey] = { ...preview, _plugin: 'parallel-channel' }; return true; },
  onPointerUp(){ return !!(parallelChannel as any)._state?.a; }
};
