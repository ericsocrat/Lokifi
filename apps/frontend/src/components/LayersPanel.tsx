...
function LayerRow({ layer }: { layer: Layer }) {
  ...
  return (
    <div
      className={
        'flex items-center gap-2 text-sm rounded border border-white/10 px-2 py-1 ' +
        (isActive ? 'bg-white/5' : '')
      }
    >
      <button
        className={'w-6 text-center rounded ' + (layer.visible ? 'opacity-100' : 'opacity-40')}
        title="Toggle visibility"
        onClick={() => s.toggleLayerVisibility(layer.id)}
      >
        👁
      </button>
      <button
        className={'w-6 text-center rounded ' + (layer.locked ? 'opacity-100' : 'opacity-40')}
        title="Toggle lock"
        onClick={() => s.toggleLayerLock(layer.id)}
      >
        🔒
      </button>
      ...
      {editing ? (
        ...
        <div className="flex-1 truncate cursor-text" onClick={() => setEditing(true)}>
          {layer.name}
        </div>
      )}
      ...
      <div className="flex items-center gap-2 w-28">
        ...
        />
        <button
          className="px-1 rounded border border-white/15 text-xs"
          onClick={() => s.moveLayer(layer.id, 'up')}
          title="Up"
        >
          ↑
        </button>
        <button
          className="px-1 rounded border border-white/15 text-xs"
          onClick={() => s.moveLayer(layer.id, 'down')}
          title="Down"
        >
          ↓
        </button>
      ...
      <button
        className={
          ...
          (isActive ? 'border-emerald-400/50' : 'border-white/15')
        }
        onClick={() => s.setActiveLayer(layer.id)}
        title="Set active"
      >
        Use
      </button>
