import type { Registered } from '@/lib/plugins/plugins';
...
export default function PluginDrawer() {
  ...
  function update(id: string, key: string, value: string) {
    ...
    setCfg(next);
    saveJSON('lokifi-plugin-cfg', next);
  }
  ...
  function exportJSON() {
    ...
    a.href = url;
    a.download = 'lokifi-plugins.json';
    a.click();
  ...
  function importJSON(file: File) {
    ...
    reader.onload = () => {
      try {
        ...
        setCfg(data);
        saveJSON('lokifi-plugin-cfg', data);
      } catch {}
    };
  ...
  return (
    <div>
              ...
              {p.defaults &&
                Object.entries(p.defaults).map(([k, v]) => (
                  <label key={k} className="flex items-center justify-between">
                    <span>{k}</span>
                    <input
                      className="ml-2 bg-transparent border px-2 py-1 rounded"
                      value={String(cfg[p.meta.id]?.[k] ?? v)}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        update(p.meta.id, k, e.target.value)
                      }
                    />
                  </label>
                ))}
      ...
      <div className="mt-4 flex gap-2">
        <button
          className="px-3 py-2 rounded-2xl border border-neutral-700 hover:border-neutral-500"
          onClick={exportJSON}
        >
          Export
        </button>
