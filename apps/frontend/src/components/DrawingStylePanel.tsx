...
export default function DrawingStylePanel() {
  ...
  const apply = (
    p: Partial<{ width: number; opacity: number; lineStyle: string; color: string; fill: string }>
  ) => setStyle(p);
    ...
    <div>
      ...
      <div className="space-y-3 text-sm">
        ...
        <label className="flex items-center justify-between">
          <span>Fill (rect)</span>
          <input
            className="ml-2 bg-transparent border px-2 py-1 rounded w-28"
            placeholder="transparent or #hex"
            value={fill}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              const v = e.target.value || 'transparent';
              setFill(v);
              apply({ fill: v });
            }}
          />
