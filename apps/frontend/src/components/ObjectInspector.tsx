...
export default function ObjectInspector() {
  ...
  const drawings = useChartStore(st => st.drawings.filter((d: Drawing) => st.selection.has(d.id))) as Drawing[];
    ...
    <div className='rounded-2xl border border-white/15 p-3 space-y-3'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-2'>
          <button className='px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10'
                  onClick={()=>s.toggleLockSelected()}>{drawings.every((d: Drawing) =>d.locked) ? 'Unlock' : 'Lock'}</button>
          <button className='px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10'
                  onClick={()=>s.toggleVisibilitySelected()}>{drawings.every((d: Drawing) =>d.hidden) ? 'Show' : 'Hide'}</button>
        </div>
      </div>
      ...
      <div className='space-y-2'>
        ...
        <input className='w-full bg-transparent border border-white/15 rounded px-2 py-1 text-sm'
               ...
               onChange={(e: React.ChangeEvent<HTMLInputElement>) =>s.renameSelected(e.target.value)} placeholder='Untitled object'/>
      </div>
      ...
      <div className='grid grid-cols-2 gap-2 text-sm'>
        <div>
          ...
          <input type='color' className='w-full h-8 bg-transparent border border-white/15 rounded'
                 ...
                 onChange={(e: React.ChangeEvent<HTMLInputElement>) =>s.setSelectedStyle({ stroke: e.target.value })}/>
        </div>
        <div>
          ...
          <input type='number' min={1} max={6} className='w-full bg-transparent border border-white/15 rounded px-2 py-1'
                 ...
                 onChange={(e: React.ChangeEvent<HTMLInputElement>) =>s.setSelectedStyle({ strokeWidth: Math.max(1, parseInt(e.target.value||'1',10)) })}/>
        </div>
        ...
        <div>
          ...
          <input type='range' min={20} max={100}
                 ...
                 onChange={(e: React.ChangeEvent<HTMLInputElement>) =>s.setSelectedStyle({ opacity: parseInt(e.target.value,10)/100 })}/>
        </div>

        <div className='col-span-2'>
          <div className='text-xs opacity-70 mb-1'>Fill (rect/ellipse/channel)</div>
          <input type='color' className='w-full h-8 bg-transparent border border-white/15 rounded'
                 value={first?.style?.fill || '#000000'}
                 onChange={(e: React.ChangeEvent<HTMLInputElement>) =>s.setSelectedStyle({ fill: e.target.value })}/>
        </div>
      </div>

      {drawings.every((d: Drawing) =>d.kind==='fib') && (
        <FibEditor />
      )}
...
function FibEditor() {
  ...
  return (
    <div className='space-y-2'>
      ...
      <input className='w-full bg-transparent border border-white/15 rounded px-2 py-1 text-sm'
             value={val} onChange={(e: React.ChangeEvent<HTMLInputElement>) =>setVal(e.target.value)} />
      <div className='flex gap-2'>
        <button className='px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10'
                onClick={()=>{
...
function parseLevels(src: string): number[] {
  return src.split(',').map((s: string) =>parseFloat(s.trim())).filter((n: number) =>!Number.isNaN(n) && n>=0 && n<=1)
}
