    ...
    <div className="rounded-2xl border border-white/15 p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() => s.toggleLockSelected()}
          >
            {drawings.every((d: Drawing) => d.locked) ? 'Unlock' : 'Lock'}
          </button>
          <button
            className="px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10"
            onClick={() => s.toggleVisibilitySelected()}
          >
            {drawings.every((d: Drawing) => d.hidden) ? 'Show' : 'Hide'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div>
          ...
          <input
            type="color"
            className="w-full h-8 bg-transparent border border-white/15 rounded"
            value={first?.style?.stroke || '#9ca3af'}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              s.setSelectedStyle({ stroke: e.target.value })
            }
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              s.setSelectedStyle({ strokeWidth: Math.max(1, parseInt(e.target.value || '1', 10)) })
            }
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              s.setSelectedStyle({ opacity: parseInt(e.target.value, 10) / 100 })
            }
          />
        </div>
        ...
        <div className="col-span-2">
          ...
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              s.setSelectedStyle({ fill: e.target.value })
            }
          />
        </div>
      </div>

      {drawings.every((d: Drawing) => d.kind === 'fib') && <FibEditor />}
...
function FibEditor() {
  ...
  const drawings = useChartStore((st) => st.drawings.filter((d) => st.selection.has(d.id)));
  const first = drawings[0];
  const levels = ((first?.kind === 'fib' ? first.fibLevels : undefined) ?? s.drawingSettings.fibDefaultLevels)
    .slice()
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-xs opacity-70">
        Comma-separated (0..1). Examples: 0, 0.236, 0.382, 0.5, 0.618, 1
      </div>
      <input
        className="w-full bg-transparent border border-white/15 rounded px-2 py-1 text-sm"
        value={val}
      ...
      <div className="flex gap-2">
        ...
        <button
          className="px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10"
          onClick={() => setVal('0, 0.236, 0.382, 0.5, 0.618, 1')}
        >
          Classic
        </button>
        <button
          className="px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10"
          onClick={() => setVal('0, 0.236, 0.382, 0.5, 0.618, 0.786, 1')}
        >
          Extended
        </button>
      </div>
