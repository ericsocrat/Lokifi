...
function AlertRow({ a }: { a: Alert }) {
  ...
  const snooze = (mins: number) =>
    s.snoozeAlert(a.id, mins ? Date.now() + mins * 60_000 : undefined);
  return (
    <div className="flex items-center justify-between text-sm rounded border border-white/10 px-2 py-1">
      <div className="truncate">
        ...
        {a.maxTriggers ? (
          <>
            {' '}
            &nbsp; <span className="opacity-60">(max {a.maxTriggers})</span>
          </>
        ) : null}
        {a.triggers ? (
          <>
            {' '}
            &nbsp; <span className="opacity-60">{a.triggers}x</span>
          </>
        ) : null}
        {a.snoozedUntil && a.snoozedUntil > Date.now() ? (
          <>
            {' '}
            &nbsp; <span className="opacity-60">snoozed</span>
          </>
        ) : null}
      </div>
      <div className="flex gap-2">
...
function MenuBtn({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <div className="px-3 py-2 hover:bg-white/10 cursor-pointer" onClick={onClick}>
      {children}
    </div>
  );
}
