...
export default function ShareBar() {
  ...
  React.useEffect(() => {
    ...
    if (snap) {
      ...
      try {
        window.__lokifi_toast?.('Loaded from share link');
      } catch {}
  ...
  const onShare = () => {
    ...
    try {
      window.__lokifi_toast?.('Share link copied');
    } catch {}
  };
  ...
  const toggleCollab = () => {
    if (collab) {
      ...
      setCollab(null);
      try {
        window.__lokifi_toast?.('Collab stopped');
      } catch {}
    } else if (room.trim()) {
      setCollab(startCollab(room.trim()));
      try {
        window.__lokifi_toast?.('Collab started');
      } catch {}
    }
  };
  ...
  return (
    <div className="rounded-2xl border border-white/15 p-3 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-sm">
        <button
          className="col-span-2 px-2 py-1 text-xs rounded border border-white/15 hover:bg-white/10"
          onClick={onShare}
        >
          Copy Share Link
        </button>
      ...
      <div className="grid grid-cols-3 gap-2 text-sm">
        ...
        >
          {collab ? 'Stop' : 'Start'} Collab
        </button>
      </div>
      <div className="text-xs opacity-60">
        Collab uses public demo server <code>demos.yjs.dev</code>. For production, point to your own
        y-websocket.
      </div>
    </div>
  );
