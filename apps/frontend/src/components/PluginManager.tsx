    ...
    <div className="rounded-2xl border border-white/15 p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => window.dispatchEvent(new CustomEvent('lokifi:install-demo-plugin'))}
        >
          Install demo
        </button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {plugins.map(
          (p: { meta: { id: string; name: string; description?: string }; enabled: boolean }) => (
            <div
              key={p.meta.id}
            ...
            >
              <div>
                <div className="font-medium">
                  {p.meta.name} <span className="opacity-60 text-xs">#{p.meta.id}</span>
                </div>
                {p.meta.description && (
                  <div className="opacity-70 text-xs">{p.meta.description}</div>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                    setPluginEnabled(p.meta.id, e.target.checked);
                    force();
                  }}
                />
              </div>
