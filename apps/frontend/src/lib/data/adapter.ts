...
export class MarketDataAdapter {
  ...
  constructor(opts?: { symbol?: string; timeframe?: string; provider?: ProviderKind }) {
    ...
    this.provider =
      opts?.provider || (process.env.NEXT_PUBLIC_DATA_PROVIDER as ProviderKind) || 'mock';
  }
  ...
  async start(): Promise<void> {
    ...
    if (this.provider === 'http') {
      ...
      try {
        const res = await fetch(
          applyQuery(url, { symbol: this.symbol, tf: this.timeframe, limit: 1000 })
        );
      ...
      this.pollTimer = setInterval(
        async () => {
          try {
            const res = await fetch(
              applyQuery(url, { symbol: this.symbol, tf: this.timeframe, limit: 2 })
            );
            const raw = await res.json();
    ...
    if (this.provider === 'ws') {
      ...
      try {
        ...
        if (url) {
          const res = await fetch(
            applyQuery(url, { symbol: this.symbol, tf: this.timeframe, limit: 600 })
          );
          const raw = await res.json();
          this.candles = normalizeCandles(raw);
          this.emit('snapshot');
...
function normalizeCandle(obj: any): Candle | null {
  ...
  if (t == null || [open, high, low, close].some((n) => typeof n !== 'number' || Number.isNaN(n)))
    return null;
...
function mockNext(prev: Candle[]): Candle[] {
  ...
  const step =
    asSec(prev[prev.length - 1].time) - asSec(prev[Math.max(0, prev.length - 2)].time) || 3600;
  ...
  if (shouldClose) {
    ...
    const next: Candle = {
      ...
      low,
      close,
      volume: Math.max(1, Math.round(100 * Math.random())),
    };
    ...
    const upd: Candle = {
      ...
      low,
      close,
      volume: last.volume + Math.round(50 * Math.random()),
    };
