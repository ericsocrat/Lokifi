...
interface PaperTradingActions {
  ...
  placeOrder: (
    order: Omit<
      PaperOrder,
      'id' | 'placedAt' | 'status' | 'filledQuantity' | 'avgFillPrice' | 'fills'
    >
  ) => Promise<string>;
    ...
    immer((set, get, _store) => ({
      ...
      resetAccount: (accountId: string) => {
        ...
        set((draft: Draft<PaperTradingStore>) => {
          ...
          if (account) {
            ...
            account.balance.availableMargin =
              account.settings.initialBalance * account.settings.maxLeverage;
        ...
        try {
          ...
          set((draft: Draft<PaperTradingStore>) => {
            ...
            if (orderToUpdate) {
              ...
              const currentPrice =
                draft.marketPrices.get(orderToUpdate.symbol) || orderToUpdate.limitPrice || 100;
              ...
              if (orderToUpdate.type === 'market') {
                ...
                executionPrice =
                  orderToUpdate.side === 'buy' ? currentPrice + slippage : currentPrice - slippage;
              } else {
      ...
      calculatePerformance: (accountId: string) => {
        ...
        const accountTrades = trades.filter(
          (t) => t.accountId === accountId && t.pnl !== undefined
        );

        const totalPnL = accountTrades.reduce((sum, t) => sum + (t.pnl || 0), 0);
        const totalReturn =
          account.settings.initialBalance > 0
      ...
      leaveChallenge: async (challengeId: string) => {
        ...
        try {
          ...
          set((draft: Draft<PaperTradingStore>) => {
            draft.participatingChallenges = draft.participatingChallenges.filter(
              (id) => id !== challengeId
            );
          });
        } catch (error) {
      ...
      liquidateAccount: async (accountId: string, reason: string) => {
        ...
        const pendingOrders = orders.filter(
          (o) => o.accountId === accountId && o.status === 'pending'
        );
      ...
      loadAccountData: async (accountId: string) => {
        ...
        try {
          ...
          set((draft: Draft<PaperTradingStore>) => {
            ...
            draft.positions = [
              ...draft.positions.filter((p) => p.accountId !== accountId),
              ...accountData.positions,
            ];
            draft.orders = [
              ...draft.orders.filter((o) => o.accountId !== accountId),
              ...accountData.orders,
            ];
            draft.trades = [
              ...draft.trades.filter((t) => t.accountId !== accountId),
              ...accountData.trades,
            ];
      ...
      exportTrades: async (accountId: string, format: 'csv' | 'json') => {
        ...
        const response = await fetch(
          `/api/paper-trading/accounts/${accountId}/export?format=${format}`
        );
      ...
      updatePositionFromOrder: (order: PaperOrder, fill: OrderFill) => {
        set((draft: Draft<PaperTradingStore>) => {
          const existingPosition = draft.positions.find(
            (p) => p.accountId === order.accountId && p.symbol === order.symbol
          );

          if (existingPosition) {
            // Update existing position
            if (
              (order.side === 'buy' && existingPosition.side === 'long') ||
              ...
              if (fill.quantity >= existingPosition.quantity) {
                ...
                closedTrade.pnlPercent =
                  closedTrade.pnl && existingPosition.avgEntryPrice > 0
                    ? (closedTrade.pnl /
                        (existingPosition.avgEntryPrice * existingPosition.quantity)) *
                      100
                    : 0;
                ...
                if (remainingQuantity > 0) {
                  ...
                  // Remove position
                  const positionIndex = draft.positions.findIndex(
                ...
                partialTrade.pnlPercent =
                  partialTrade.pnl && existingPosition.avgEntryPrice > 0
...
export const useAccountPositions = (accountId?: string) =>
  usePaperTradingStore((draft: Draft<PaperTradingStore>) => {
    ...
    return targetAccountId ? draft.positions.filter((p) => p.accountId === targetAccountId) : [];
  });

export const useAccountOrders = (accountId?: string) =>
  usePaperTradingStore((draft: Draft<PaperTradingStore>) => {
    const targetAccountId = accountId || draft.activeAccountId;
    return targetAccountId ? draft.orders.filter((o) => o.accountId === targetAccountId) : [];
  });
