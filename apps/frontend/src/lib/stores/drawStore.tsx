...
function key(sym: string, tf: string) {
  return `${LS_PREFIX}.${sym}.${tf}`;
}
function load(sym: string, tf: string): Shape[] {
  try {
    const raw = localStorage.getItem(key(sym, tf));
    return raw ? (JSON.parse(raw) as Shape[]) : [];
  } catch {
    return [];
  }
}
function save(sym: string, tf: string, shapes: Shape[]) {
  try {
    localStorage.setItem(key(sym, tf), JSON.stringify(shapes));
  } catch {}
}
...
export const drawStore = {
  ...
  undo() {
    ...
    redoStack.push(_state.shapes);
    const prev = undoStack.pop()!;
    _state = { ..._state, shapes: prev };
    const sym = symbolStore.get();
    const tf = timeframeStore.get();
    save(sym, tf, _state.shapes);
    listeners.forEach((l: any) => l(_state));
