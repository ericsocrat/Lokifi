...
export function setMappers(m: Partial<Mappers>) {
  _mappers = { ..._mappers, ...m };
}
export function yToPrice(y: number): number | null {
  return _mappers.yToPrice ? _mappers.yToPrice(y) : null;
}
export function priceToY(p: number): number | null {
  return _mappers.priceToY ? _mappers.priceToY(p) : null;
}
export function xToTime(x: number): number | Date | string | null {
  return _mappers.xToTime ? _mappers.xToTime(x) : null;
}
export function timeToX(t: number | Date | string): number | null {
  return _mappers.timeToX ? _mappers.timeToX(t) : null;
}
...
export function snapPxToGrid<T extends Point>(p: T, step: number, enabled: boolean): T {
  if (!enabled) return p;
  return { ...p, x: Math.round(p.x / step) * step, y: Math.round(p.y / step) * step } as T;
}
...
export function setVisibleBarCoords(xs: number[]) {
  _visibleBarXs = xs.slice().sort((a, b) => a - b);
}
...
export function magnetXToBars(x: number, tolerancePx: number): number {
  ...
  let lo = 0,
    hi = _visibleBarXs.length - 1;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (_visibleBarXs[mid] < x) lo = mid + 1;
    else hi = mid;
  }
  const idx = lo;
  const cands = [idx - 1, idx, idx + 1]
    .filter((i) => i >= 0 && i < _visibleBarXs.length)
    .map((i) => _visibleBarXs[i]);
  let best = x,
    dBest = Number.POSITIVE_INFINITY;
