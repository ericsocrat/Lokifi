...
export function toast(text: string, ttl = 1200) {
  ...
  listeners.forEach((fn: (msg: ToastMsg) => void) => fn(msg))
}
...
export function ToastHost() {
  ...
  React.useEffect(() => {
    const on = (m: ToastMsg) => {
      ...
      setTimeout(() => {
        setItems(prev => prev.filter((x: ToastMsg) => x.id !== m.id))
      }, m.ttl)
    }
    listeners.push(on)
    return () => { listeners = listeners.filter((l: (msg: ToastMsg) => void) => l !== on) }
  }, [])
  return (
    <div className="pointer-events-none fixed z-[999] bottom-4 right-4 flex flex-col gap-2">
      {items.map((m: ToastMsg) => (
        <div key={m.id}
