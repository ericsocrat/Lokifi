...
export function describeDrawing(d: Drawing, cfg: LabelConfig) {
  try {
    switch (d.kind) {
      case 'hline':
        return hlineLabel(d, cfg);
      case 'vline':
        return vlineLabel(d, cfg);
      case 'trendline':
      case 'ray':
      case 'arrow':
        return lineLabel(d, cfg);
      case 'rect':
        return rectLabel(d, cfg);
      case 'ruler':
        return rulerLabel(d, cfg);
      case 'text':
        return null;
      default:
        return null;
...
function vlineLabel(_d: Drawing, _cfg: LabelConfig) {
  return null;
}
...
function lineLabel(d: Drawing, cfg: LabelConfig) {
  ...
  if (p1 != null && p2 != null) {
    ...
    if (cfg.showPercent && p1 !== 0)
      parts.push(`${dp >= 0 ? '+' : ''}${fmt((dp / Math.abs(p1)) * 100, 2)}%`);
  }
  if (cfg.showAngle) {
