...
export default function LeftDock() {
  ...
  const Btn = ({
    ...
    <button
      title={title}
      onClick={onClick}
      className={`p-2 rounded-xl ${active ? 'bg-electric/30 border border-electric' : 'border border-neutral-700 hover:bg-neutral-800'} flex items-center justify-center`}
    >
      {children}
    </button>
  );
    ...
    <div className="absolute left-2 top-16 z-20 flex flex-col gap-2 p-2 rounded-2xl bg-neutral-900/90 border border-neutral-800">
      ...
      {EXPERIMENTAL_PLUGINS && (
        <>
          ...
          <Btn
            title="Ruler (R)"
            active={activePlugin === 'ruler-measure'}
            onClick={() =>
              pluginManager.setActiveTool(activePlugin === 'ruler-measure' ? null : 'ruler-measure')
            }
          >
            <Ruler size={16} />
          </Btn>
          <Btn
            title="Channel (C)"
            active={activePlugin === 'parallel-channel'}
            onClick={() =>
              pluginManager.setActiveTool(
                activePlugin === 'parallel-channel' ? null : 'parallel-channel'
              )
            }
          >
            <Waves size={16} />
          ...
          <Btn
            title="Channel 3pt (Shift+C)"
            active={activePlugin === 'parallel-channel-3pt'}
            onClick={() =>
              pluginManager.setActiveTool(
                activePlugin === 'parallel-channel-3pt' ? null : 'parallel-channel-3pt'
              )
            }
          >
            <Waves size={16} />
          ...
          <Btn
            title="Fib+ (F)"
            active={activePlugin === 'fib-extended'}
            onClick={() =>
              pluginManager.setActiveTool(activePlugin === 'fib-extended' ? null : 'fib-extended')
            }
          >
            <Brackets size={16} />
          </Btn>
      ...
      <div className="h-px bg-neutral-800 my-1" />
      <Btn title="Settings (Gear)" active={open} onClick={() => setOpen((x) => !x)}>
        <Settings size={16} />
      </Btn>
