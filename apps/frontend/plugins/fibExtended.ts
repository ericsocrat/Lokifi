...
function levels() {
  ...
  if (s.fibPreset === 'Aggressive')
    return [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1, 1.272, 1.414, 1.618, 2, 2.618, 3.618, 4.236];
  if (s.fibPreset === 'Custom')
    return s.fibCustomLevels?.length ? s.fibCustomLevels : [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1];
...
export const fibExtended: ToolPlugin = {
  ...
  onPointerMove(e, ctx) {
    const st: any = (fibExtended as any)._state;
    if (!st?.a) return false;
    const { snapped } = ctx.xy(e);
    (globalThis as any)[ghostKey] = { type: 'fib', a: st.a, b: snapped, _plugin: 'fib-extended' };
    return true;
  },
  onPointerUp() {
