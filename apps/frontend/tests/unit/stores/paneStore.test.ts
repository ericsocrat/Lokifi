import { type Pane, usePaneStore } from '@/lib/stores/paneStore';
...
describe('PaneStore', () => {
  ...
  it('should add indicator pane', () => {
    ...
    const indicatorPane = panes.find((pane: Pane) => pane.id === newPaneId);
  ...
  it('should add indicator to existing pane', () => {
    ...
    const updatedPricePane = updatedPanes.find((pane: Pane) => pane.id === pricePane.id);
  ...
  it('should move indicator between panes', () => {
    ...
    const updatedPanes = usePaneStore.getState().panes;
    const updatedPricePane = updatedPanes.find((pane: Pane) => pane.id === pricePane.id);
    const updatedIndicatorPane = updatedPanes.find((pane: Pane) => pane.id === indicatorPaneId);
  ...
  it('should remove pane', () => {
    ...
    expect(panes).toHaveLength(1);
    expect(panes.find((pane: Pane) => pane.id === indicatorPaneId)).toBeUndefined();
  });
