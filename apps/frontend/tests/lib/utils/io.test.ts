  ...
  describe('exportPngFromRoot', () => {
    ...
    it('should use max dimensions from canvases', async () => {
      ...
      createElementSpy.mockImplementation((tag) => {
        if (tag === 'canvas') {
          return {
            get width() { return outputCanvasWidth; },
            set width(v) { outputCanvasWidth = v; },
            get height() { return outputCanvasHeight; },
            set height(v) { outputCanvasHeight = v; },
            getContext: () => mockContext,
            toBlob: vi.fn((cb) => cb(new Blob(['']))),
          } as unknown as HTMLCanvasElement;
        }
        return {
          href: '', download: '', click: vi.fn(), remove: vi.fn(),
        } as unknown as HTMLAnchorElement;
      });
    ...
    it('should use default filename', async () => {
      ...
      createElementSpy.mockImplementation((tag) => {
        ...
        return {
          ...
          set download(v) { downloadFilename = v; },
          click: vi.fn(),
          remove: vi.fn(),
        } as unknown as HTMLAnchorElement;
      });
