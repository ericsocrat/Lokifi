...
class DatabaseSetup:
        ...
        try:
            ...
            if "lokifi-postgres" in result.stdout:
                ...
                start_result = subprocess.run(
                    ["docker", "start", "lokifi-postgres"], capture_output=True, text=True
                )
                ...
                create_cmd = [
                    ...
                    "5432:5432",
                    "-v",
                    "lokifi_postgres_data:/var/lib/postgresql/data",
                    "postgres:15-alpine",
                ]
    ...
    def setup_database_config(self) -> str:
        ...
        if self.check_docker_available():
            if self.start_postgres_docker():
                database_url = (
                    "postgresql+asyncpg://lokifi:lokifi_dev_password@localhost:5432/lokifi"
                )
            ...
            if database_url.startswith("postgresql"):
                ...
                if "localhost:5432" in database_url:
                    if "lokifi:" in database_url:
                        # Docker setup
                        db_password = os.getenv("POSTGRES_PASSWORD", "lokifi_dev_password")
    ...
    async def run_database_migration(self):
        ...
        try:
            ...
            # Run alembic upgrade
            result = subprocess.run(
                ["alembic", "upgrade", "head"], cwd=self.backend_dir, capture_output=True, text=True
            )
