    ...
    async def run_migration():
        ...
        try:
            ...
            result = await migration_service.migrate_database(
                source_url=source, target_url=target, batch_size=batch_size
            )
...
def metrics():
    ...
    async def show_metrics():
        ...
        if metrics.oldest_message_date:
            click.echo(
                f"Oldest Message: {metrics.oldest_message_date.strftime('%Y-%m-%d %H:%M:%S')}"
            )
        if metrics.newest_message_date:
            click.echo(
                f"Newest Message: {metrics.newest_message_date.strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    async def run_maintenance():
        ...
        for operation, stats in results.items():
            if operation == "archive":
                click.echo(
                    f"   📁 Archived: {stats.messages_archived} messages, {stats.threads_archived} threads"
                )
            elif operation == "compress":
                click.echo(
                    f"   🗜️  Compressed: {stats.messages_compressed} messages, freed {stats.space_freed_mb:.2f} MB"
                )
