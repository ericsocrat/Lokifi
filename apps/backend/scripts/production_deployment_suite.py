...
try:
    ...
    print(
        "Install missing dependencies: pip install docker psutil aiofiles httpx prometheus_client"
    )
    sys.exit(1)
    ...
    def setup_metrics(self):
        ...
        self.metrics = {
            "http_requests_total": Counter(
                "lokifi_http_requests_total",
                "Total HTTP requests",
                ["method", "endpoint", "status"],
                registry=self.registry,
            ),
            "http_request_duration": Histogram(
                "lokifi_http_request_duration_seconds",
                "HTTP request duration",
                ["method", "endpoint"],
                registry=self.registry,
            ),
            "active_connections": Gauge(
                "lokifi_active_connections", "Active connections", registry=self.registry
            ),
            "database_connections": Gauge(
                "lokifi_database_connections", "Database connections", registry=self.registry
            ),
            "memory_usage": Gauge(
                "lokifi_memory_usage_bytes", "Memory usage in bytes", registry=self.registry
            ),
            "cpu_usage": Gauge(
                "lokifi_cpu_usage_percent", "CPU usage percentage", registry=self.registry
            ),
            "disk_usage": Gauge(
                "lokifi_disk_usage_bytes", "Disk usage in bytes", ["path"], registry=self.registry
            ),
        }
...
"""
            ...
            compose_content = {
                ...
                "networks": {"lokifi_network": {"driver": "bridge"}},
            }
        ...
        try:
            ...
            prometheus_config = {
                ...
                "scrape_configs": [
                    {
                        "job_name": "lokifi-api",
                        "static_configs": [{"targets": ["lokifi-api:8000"]}],
...
"""
    ...
    async def collect_system_metrics(self) -> dict[str, Any]:
        ...
        try:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            metrics_response = await client.get(
                                "http://localhost:8002/metrics", timeout=5
                            )
            ...
            metrics_file = (
                self.logs_dir / f"metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    async def health_check_all_services(self) -> dict[str, Any]:
        ...
        for service_name, service_url in services_to_check:
            try:
                ...
                health_status["services"][service_name] = {"status": "down", "error": str(e)}
        ...
        self.print_info(
            f"Overall health: {health_status['overall_status']} ({healthy_services}/{total_services} services)"
        )
    ...
    async def generate_monitoring_report(self) -> dict[str, Any]:
        ...
        if health_status["health_percentage"] < 100:
            ...
            report["alerts"].append(
                {
                    "level": "warning",
                    "message": f"Only {health_status['health_percentage']:.0f}% of services healthy",
                }
            )
    ...
    async def run_production_setup(self) -> bool:
        ...
        print(
            f"{Colors.WHITE}Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.END}"
        )
