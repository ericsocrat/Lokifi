        ...
        for symbol, period in test_cases:
            try:
                ...
                if data['count'] > 0:
                    first = data['data'][0]
                    last = data['data'][-1]
                    first_date = datetime.fromtimestamp(first['timestamp']).strftime('%Y-%m-%d %H:%M')
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for symbol, period in test_cases:
            try:
                ...
                if data['count'] > 0:
                    candle = data['data'][0]
                    print(f"   First candle: O=${candle['open']:,.2f} H=${candle['high']:,.2f} " +
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            for i, crypto in enumerate(data['cryptos'][:5], 1):
                print(f"   {i}. {crypto['name']} ({crypto['symbol']}) - ${crypto['current_price']:,.2f}")
        except Exception as e:
        ...
        try:
            ...
            print(f"   Sample: BTC → {mapping.get('BTC', 'N/A')}, ETH → {mapping.get('ETH', 'N/A')}")
        except Exception as e:
