...
class AdvancedStressTester:
    ...
    async def initialize(self):
        ...
        self.session = aiohttp.ClientSession(
            connector=connector, timeout=timeout, headers={"User-Agent": "Lokifi-StressTester/1.0"}
        )
    ...
    async def run_load_test(self, config: LoadTestConfig) -> StressTestResult:
                ...
                try:
                    ...
                    if config.method.upper() == "GET":
                        ...
                        async with session.post(
                            url, json=config.payload, headers=config.headers
                        ) as response:
                            await response.text()
                            success = 200 <= response.status < 400
        ...
        print(
            f"   📊 Requests: {successful_requests}/{total_requests_made} success ({error_rate:.1f}% error)"
        )
        print(f"   🚀 RPS: {rps:.1f}")
    ...
    async def run_websocket_load_test(
        self, concurrent_connections: int, duration_seconds: int
    ) -> StressTestResult:
                    ...
                    while time.time() < end_time:
                        try:
                            ...
                            message = {
                                "type": "ping",
                                "timestamp": time.time(),
                                "data": f"test-{random.randint(1, 1000)}",
                            }
                            send_time = time.time()
        ...
        while time.time() < end_time:
            ...
            print(
                f"   📊 Connections: {successful_connections}/{concurrent_connections}, Messages: {messages_sent}→{messages_received}"
            )
    ...
    def generate_comprehensive_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "recommendations": self._generate_recommendations(
                overall_success_rate, avg_rps, avg_response_time, avg_memory_growth
            ),
    ...
    def _generate_recommendations(
        self, success_rate: float, avg_rps: float, avg_response_time: float, memory_growth: float
    ) -> list[str]:
        ...
        if success_rate < 95:
            recommendations.append(
                "🔴 Success rate below 95% - investigate error handling and system stability"
            )

        if avg_response_time > 1000:
            recommendations.append(
                "🟡 Average response time > 1s - consider caching, database optimization, or scaling"
            )

        if avg_rps < 50:
            recommendations.append(
                "🟡 Low throughput - consider connection pooling, async optimization, or horizontal scaling"
            )
