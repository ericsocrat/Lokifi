...
def test_profile_endpoints():
    ...
    response = requests.put(
        f"{BASE_URL}/api/profile/settings/user", json=settings_update, cookies=cookies
    )
    if response.status_code == 200:
    ...
    print("\n🔔 Step 7: Update notification preferences...")
    notif_update = {"email_follows": False, "email_messages": True, "push_enabled": False}

    response = requests.put(
        f"{BASE_URL}/api/profile/settings/notifications", json=notif_update, cookies=cookies
    )
    if response.status_code == 200:
        ...
        print(
            f"❌ Notification preferences update failed: {response.status_code} - {response.text}"
        )
    ...
    if response.status_code == 200:
        ...
        if search_results["profiles"]:
            ...
            print(
                f"   First Result: {first_profile['display_name']} (@{first_profile['username']})"
            )
    else:
