...
def test_follow_graph():
    ...
    # Bob
    bob_cookies = create_test_user("bob@example.com", "bob_test", "Bob Test User")
    if not bob_cookies:
    ...
    response = requests.post(
        f"{BASE_URL}/api/follow/follow", json=follow_data, cookies=alice_cookies
    )
    if response.status_code == 200:
    ...
    response = requests.post(
        f"{BASE_URL}/api/follow/follow", json=follow_data, cookies=charlie_cookies
    )
    if response.status_code == 200:
    ...
    response = requests.get(
        f"{BASE_URL}/api/follow/{alice_user_id}/followers", cookies=alice_cookies
    )
    if response.status_code == 200:
        ...
        if followers_data["followers"]:
            ...
            print(
                f"   First Follower: {first_follower['display_name']} (follows_you: {first_follower['follows_you']})"
            )
    else:
    ...
    if response.status_code == 200:
        ...
        if following_data["following"]:
            ...
            print(
                f"   First Following: {first_following['display_name']} (mutual: {first_following['mutual_follow']})"
            )
    else:
    ...
    response = requests.get(
        f"{BASE_URL}/api/follow/mutual/{charlie_user_id}", cookies=alice_cookies
    )
    if response.status_code == 200:
    ...
    response = requests.delete(
        f"{BASE_URL}/api/follow/unfollow", json=unfollow_data, cookies=alice_cookies
    )
    if response.status_code == 200:
    ...
    response = requests.post(
        f"{BASE_URL}/api/follow/bulk/follow", json=bulk_follow_data, cookies=alice_cookies
    )
    if response.status_code == 200:
