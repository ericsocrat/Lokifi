...
from app.models.conversation import Conversation, ConversationParticipant, Message, MessageReceipt, ContentType
...
class TestDMConversationCreation:
    ...
    @pytest.mark.skip(reason="SQLAlchemy model circular dependency: MessageReaction not found during model loading")
    @pytest.mark.asyncio
    async def test_get_or_create_dm_returns_existing_conversation(
        ...
        """Test that existing DM conversation is returned (SKIPPED: model dependency issue)"""
        ...
        # Call method
        with patch.object(
            conversation_service, '_build_conversation_response', 
...
class TestMessageSending:
    ...
    async def test_send_message_creates_message_and_receipt(
        ...
        """Test successful message creation (SKIPPED: model dependency issue)"""
        ...
        with patch.object(
            conversation_service, '_build_message_response',
            new_callable=AsyncMock
        ) as mock_build:
...
class TestMessageRetrieval:
    ...
    async def test_get_messages_pagination(
        ...
        """Test message pagination works correctly (SKIPPED: model dependency issue)"""
        # Mock participant query - user is participant
        mock_participant = MagicMock(is_active=True)
        ...
        mock_messages_result.scalars().all.return_value = mock_messages
        ...
        # Verify pagination math (not checking actual result, just that method ran)
...
class TestConversationListing:
    ...
    async def test_get_user_conversations_pagination(
        ...
        """Test conversation listing with pagination (SKIPPED: model dependency issue)"""
        # Mock conversations query
        mock_conversations = [MagicMock() for _ in range(5)]
        mock_conv_result = MagicMock()
        mock_conv_result.scalars().all.return_value = mock_conversations
        ...
        # Verify response builder called for each conversation
        assert mock_build.call_count == len(mock_conversations)
...
class TestConversationServiceEdgeCases:
    ...
    async def test_invalid_user_id_format(
        ...
        """Test handling of invalid UUID format (SKIPPED: model dependency issue)"""
        invalid_id = "not-a-valid-uuid"
        
        # Should raise ValueError or TypeError
        with pytest.raises((ValueError, TypeError, HTTPException)):
            await conversation_service.get_user_conversations(
