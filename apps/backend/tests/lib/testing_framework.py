...
class AdvancedTestFramework:
                ...
                async with engine.begin() as conn:
                    ...
                    if len(found_tables) >= 2:
                        ...
                        self.print_test(
                            "Table Structure",
                            "FAIL",
                            f"Missing expected tables. Found: {', '.join(tables)}",
                        )
            ...
            async with httpx.AsyncClient() as client:
                ...
                if successful >= 8:  # Allow 80% success rate
                    self.print_test(
                        "Concurrent Requests",
                        "PASS",
                        f"{successful}/10 successful in {end_time - start_time:.3f}s",
                    )
                    passed += 1
                else:
                    self.print_test(
    ...
    async def test_social_features(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                try:
                    response = await client.get(
                        f"{self.base_url}/social/followers", headers=headers
                    )
        ...
        self.record_test(
            "social",
            "overall",
            "PASS" if success_rate >= 50 else "FAIL",
            {"passed": passed, "total": total, "success_rate": success_rate},
        )
    ...
    def generate_test_report(self) -> dict[str, Any]:
        ...
        if overall_success_rate < 90:
            failed_categories = [
                cat
                for cat, tests in self.test_results.items()
