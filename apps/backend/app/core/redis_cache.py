...
class RedisCache:
    ...
    async def set(self, key: str, value: Any, ttl: int | None = None) -> bool:
        ...
        try:
            ...
            cached_data = {"data": value, "__cached_at__": time.time()}
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args: Any, **kwargs: Any):
            ...
            if invalidate_on_mutation and request.method in ["POST", "PUT", "DELETE", "PATCH"]:
                ...
                logger.info(
                    f"Cache invalidated for prefix {prefix} due to {request.method} request"
                )
                return await func(*args, **kwargs)
            ...
            if result is not None:
                ...
                logger.debug(
                    f"Cached result for {func.__name__}, execution: {execution_time:.1f}ms"
                )
...
def cache_public_data(ttl: int = 1800):
    ...
    return redis_cache(ttl=ttl, prefix="public", vary_on_user=False)
...
def cache_notifications(ttl: int = 120):
    ...
    return redis_cache(
        ttl=ttl, prefix="notifications", vary_on_user=True, invalidate_on_mutation=True
    )


def cache_ai_responses(ttl: int = 900):
    """Cache AI responses for 15 minutes"""
    return redis_cache(ttl=ttl, prefix="ai_responses", vary_on_user=True)


def cache_market_data(ttl: int = 60):
    """Cache market data for 1 minute (frequently updated)"""
    return redis_cache(ttl=ttl, prefix="market_data", vary_on_user=False)
...
async def warm_cache():
    ...
    try:
        ...
        cache_warmed = {"cache_warmed_at": time.time(), "status": "warmed"}
