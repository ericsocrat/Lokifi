...
async def health_check(service: SmartPriceService = Depends(get_price_service)):
    try:
        ...
        return HealthResponse(
            status="healthy", redis_connected=redis_connected, providers=providers
        )
    ...
    try:
        ...
        for symbol in symbols:
            if symbol in price_data_dict:
                ...
                data[symbol] = PriceResponse(
                    symbol=pd.symbol,
                    price=pd.price,
                    change=pd.change,
                    change_percent=pd.change_percent,
                    volume=pd.volume,
...
async def get_performance_stats():
    ...
    try:
        ...
        return {
            ...
            "summary": {
                "total_operations": performance_metrics.total_requests
                + crypto_metrics.total_fetches,
...
async def get_price_history(
    ...
    period: PeriodType = Query(
        default="1m", description="Time period: 1d, 1w, 1m, 3m, 6m, 1y, 5y, all"
    ),
