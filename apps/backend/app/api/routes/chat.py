...
async def tool_create_price_alert(
    symbol: str, direction: str, price: float, authorization: str | None
) -> dict[str, Any]:
    payload = CreateAlertModel(
...
async def chat(req: ChatRequest, authorization: str | None = Header(None)) -> dict[str, Any]:
    ...
    if text.startswith("/price"):
        ...
        if len(parts) >= 2:
            ...
            return {
                "mode": "command",
                "result": data,
                "answer": f"{data['symbol']} {data['timeframe']} price: {data['price']}",
            }
        else:
    ...
    if text.startswith("/alert"):
        ...
        return {
            "mode": "command",
            "result": data,
            "answer": f"Alert created on {sym} {direction} {price}",
        }
    ...
    if text.startswith("/portfolio"):
        ...
        return {
            "mode": "command",
            "result": data,
            "answer": f"Your portfolio value: {data['total_value']} (P/L {data['total_pl']} / {data['total_pl_pct']}%)",
        }
        ...
        if first and first.get("choices"):
            ...
            if "tool_calls" in msg:
                for tc in msg["tool_calls"]:
                    ...
                    if fn == "get_price":
                        out = await tool_get_price(
                            args.get("symbol", ""), args.get("timeframe", "1h")
                        )
                        return {
                            "mode": "tool",
                            "tool": fn,
                            "args": args,
                            "result": out,
                            "answer": f"{out['symbol']} {out['timeframe']} price: {out['price']}",
                        }
                        ...
                        return {
                            ...
                            "args": args,
                            "result": out,
                            "answer": f"Your portfolio value: {out['total_value']} (P/L {out['total_pl']} / {out['total_pl_pct']}%)",
                        }
                        ...
                        return {
                            ...
                            "args": args,
                            "result": out,
                            "answer": f"Alert created on {args['symbol']} {args['direction']} {args['price']}",
                        }
    ...
    if "price" in lower:
        ...
        return {
            "mode": "offline",
            "answer": f"{out['symbol']} 1h price: {out['price']}",
            "result": out,
        }
    ...
    return {
        "mode": "offline",
        "answer": "Try /price BTCUSD, /alert BTCUSD above 45000, or /portfolio (requires login).",
    }
