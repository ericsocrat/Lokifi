...
class VersionDetectionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        When version is specified in URL path (e.g. /api/v1/example/version),
        the version prefix is stripped so routing matches the registered routes
        (e.g. /api/example/version).
        """
        ...
        # e.g. /api/v1/example/version -> /api/example/version
