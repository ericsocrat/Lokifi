...
class ConversationExporter:
    ...
    def export_conversations(
        self, user_id: int, options: ExportOptions, db: Session | None = None
    ) -> str | bytes:
    ...
    def _get_conversations_data(
        ...
        if options.date_range:
            ...
            threads_query = threads_query.filter(
                AIThread.created_at >= start_date, AIThread.created_at <= end_date
            )
    ...
    def _export_json(self, conversations: list[dict[str, Any]], options: ExportOptions) -> str:
        """Export as JSON."""
        export_data = {
            "exported_at": datetime.now(timezone.timezone.utc).isoformat(),
            "format": "json",
    ...
    def _export_markdown(self, conversations: list[dict[str, Any]], options: ExportOptions) -> str:
        ...
        output.append(f"\nExported at: {datetime.now(timezone.timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _export_html(self, conversations: list[dict[str, Any]], options: ExportOptions) -> str:
        ...
        output.append(
            ".user { background: #e8f4ff; padding: 10px; border-radius: 5px; margin: 10px 0; }"
        )
        output.append(
        ...
        output.append(
            f"<p><strong>Exported at:</strong> {datetime.now(timezone.timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}</p>"
        )
    ...
    def _export_xml(self, conversations: list[dict[str, Any]], options: ExportOptions) -> str:
        ...
        root.set("exported_at", datetime.now(timezone.timezone.utc).isoformat())
    ...
    def _export_txt(self, conversations: list[dict[str, Any]], options: ExportOptions) -> str:
        ...
        output.append("AI CONVERSATIONS EXPORT")
        output.append("=" * 50)
        output.append(f"Exported at: {datetime.now(timezone.timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}")
        output.append(f"Total conversations: {len(conversations)}")
        output.append("")
...
class ConversationImporter:
    ...
    def _import_json(
        self, user_id: int, content: str | bytes, merge_strategy: str, db: Session
    ) -> dict[str, Any]:
        ...
        for conv_data in data["conversations"]:
            try:
                ...
                if existing_thread and merge_strategy == "overwrite":
                    ...
                    thread.updated_at = datetime.now(timezone.timezone.utc)
                    ...
                    thread = AIThread(
                        ...
                        created_at=(
                            ...
                            else datetime.now(timezone.timezone.utc)
                        ),
                        updated_at=(
