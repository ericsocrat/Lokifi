...
class NotificationService:
            ...
            async for session in db_manager.get_session():
                ...
                if not skip_preferences:
                    preferences = await self._get_user_preferences(
                        session, notification_data.user_id
                    )
    ...
    async def create_batch_notifications(
        self, notifications_data: list[NotificationData], batch_id: str | None = None
    ) -> list[Notification]:
        ...
        await self._emit_event(
            NotificationEvent.BATCH_PROCESSED,
            {
                "batch_id": batch_id,
        ...
        logger.info(
            f"Batch created {len(created_notifications)}/{len(notifications_data)} notifications"
        )
            ...
            async for session in db_manager.get_session(read_only=True):
                ...
                query = query.where(
                    or_(
                        ...
                        Notification.expires_at > datetime.now(timezone.timezone.utc),
                    )
                )
    ...
    async def _deliver_notification(
        self, notification: Notification, notification_data: NotificationData
    ) -> None:
