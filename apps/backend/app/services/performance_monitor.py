...
class HealthCheck:
    ...
    timestamp: datetime = field(default_factory=lambda: datetime.now(timezone.timezone.utc))
...
class PerformanceMonitor:
    ...
    def record_metric(
        ...
        metric = PerformanceMetric(
            name=name, value=value, unit=unit, timestamp=datetime.now(timezone.timezone.utc), tags=tags or {}
        )
    ...
    def record_websocket_connection(self, user_id: uuid.UUID):
        ...
        self.websocket_connections[user_id] = datetime.now(timezone.timezone.utc)
    ...
    def record_websocket_disconnection(self, user_id: uuid.UUID):
        ...
        if user_id in self.websocket_connections:
            connection_time = datetime.now(timezone.timezone.utc) - self.websocket_connections[user_id]
            self.record_metric(
    ...
    def get_metrics_summary(self, minutes_back: int = 10) -> dict[str, Any]:
        ...
        cutoff_time = datetime.now(timezone.timezone.utc) - timedelta(minutes=minutes_back)
