...
class AuthService:
    ...
    async def register_user(self, user_data: UserRegisterRequest) -> dict[str, Any]:
        ...
        profile = Profile(
            ...
            is_public=True,
            follower_count=0,  # Explicitly set defaults for testability
            following_count=0,  # Explicitly set defaults for testability
        )

        self.db.add(profile)

        # Create notification preferences
        notification_prefs = NotificationPreference(id=uuid.uuid4(), user_id=user.id)
