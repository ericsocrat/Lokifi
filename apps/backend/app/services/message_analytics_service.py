...
class MessageAnalyticsService:
    ...
    async def get_user_message_stats(
        ...
        cutoff_date = datetime.now(timezone.timezone.utc) - timedelta(days=days_back)
    ...
    async def get_platform_statistics(self) -> dict[str, Any]:
        ...
        cutoff_date = datetime.now(timezone.timezone.utc) - timedelta(days=30)
    ...
    async def get_trending_conversations(
        ...
        trending_stmt = (
            ...
            .where(
                and_(
                    Message.created_at >= datetime.now(timezone.timezone.utc) - timedelta(hours=24),
                    ~Message.is_deleted,
                )
            )
