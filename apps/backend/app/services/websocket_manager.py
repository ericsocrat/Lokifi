...
class ConnectionManager:
    ...
    async def broadcast_new_message(
        self, message_response: MessageResponse, participant_ids: set[uuid.UUID]
    ):
    ...
    async def broadcast_typing_indicator(
        ...
        try:
            ...
            await self.send_to_conversation_participants(
                message_json, participant_ids, exclude_user_id=user_id
            )
    ...
    async def broadcast_read_receipt(
        ...
        try:
            read_msg = MessageReadNotification(
                ...
                read_at=datetime.now(timezone.timezone.utc),
            )
                    ...
                    json.dumps(
                        {
                            ...
                            "read_at": datetime.now(timezone.timezone.utc).isoformat(),
                            "participant_ids": [str(uid) for uid in participant_ids],
                        }
                    ),
                )
    ...
    async def close(self):
        ...
        try:
            if (
                ...
                and hasattr(self.redis_client, "close")
            ):
