...
class J53PerformanceMonitor:
    ...
    async def check_database_health(self) -> dict[str, Any]:
        ...
        try:
            async for session in db_manager.get_session(read_only=True):
                ...
                health_data["connection_healthy"] = (
                    connection_time < MetricThreshold.RESPONSE_TIME_WARNING.value
                )
                ...
                health_data["size_healthy"] = (
                    db_size_mb < MetricThreshold.DATABASE_SIZE_WARNING.value
                )
    ...
    def calculate_system_health(self) -> SystemHealth:
        ...
        info_count = len(
            [
                a
                for a in self.active_alerts.values()
                if a.severity == AlertSeverity.INFO and not a.resolved
            ]
        )
        ...
        older_alerts = [
            ...
            if datetime.now() - timedelta(hours=48)
            <= a.timestamp
    ...
    async def run_monitoring_cycle(self) -> dict[str, Any]:
        ...
        logger.info(
            f"📊 Monitoring complete: {health.status} status, score: {health.score:.1f}/100, "
    ...
    async def recommend_scaling_actions(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for alert in metrics_report["active_alerts"]:
                ...
                elif alert["category"] == "Performance" and alert["severity"] in [
                    "critical",
                    "warning",
                ]:
                    recommendations.append(
