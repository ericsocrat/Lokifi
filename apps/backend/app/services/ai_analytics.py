...
class AIAnalyticsService:
    ...
    async def get_user_insights(self, user_id: int, days_back: int = 90) -> UserInsights:
        ...
        with self.session_factory() as db:
            ...
            total_threads = (
                db.query(AIThread)
                .filter(AIThread.user_id == user_id, AIThread.created_at >= start_date)
                .count()
            )
            ...
            preferred_providers = [
                row[0]
                for row in db.query(AIMessage.provider)
                .join(AIThread)
                .filter(
    ...
    async def _extract_conversation_topics(
        self, db: Session, start_date: datetime, user_id: int | None = None
    ) -> list[dict[str, Any]]:
        ...
        query = (
            ...
            .join(AIThread)
            .filter(AIThread.created_at >= start_date, AIMessage.role == "user")
        )
        if user_id:
    ...
    async def _extract_user_topics(
        self, db: Session, user_id: int, start_date: datetime
    ) -> list[str]:
