...
class ProfileService:
    ...
    async def update_profile(
        self, user_id: uuid.UUID, profile_data: ProfileUpdateRequest
    ) -> ProfileResponse:
        """Update user profile."""
        ...
        if profile_data.avatar_url is not None:
            update_data["avatar_url"] = (
                str(profile_data.avatar_url) if profile_data.avatar_url else None
            )
    ...
    async def update_user_settings(
        self, user_id: uuid.UUID, settings_data: UserSettingsUpdateRequest
    ) -> UserSettingsResponse:
    ...
    async def update_notification_preferences(
        self, user_id: uuid.UUID, prefs_data: NotificationPreferencesUpdateRequest
    ) -> NotificationPreferencesResponse:
    ...
    async def get_public_profile(
        self, profile_id: uuid.UUID, current_user_id: uuid.UUID | None = None
    ) -> PublicProfileResponse:
        ...
        if current_user_id:
            ...
            is_following = await follow_service.is_following(
                follower_id=current_user_id, followee_id=profile.user_id
            )
