...
class OptimizationRecommendation:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {**asdict(self), "metadata": self.metadata or {}}
...
class DatabaseOptimizer:
    ...
    def _estimate_index_usage(self, query: str) -> bool:
        ...
        return "where" in query_lower and any(
            col in query_lower for col in ["id", "user_id", "created_at"]
        )
        ...
        for metric in self.query_metrics:
            if metric.full_table_scan and metric.execution_time_ms > 50:
                ...
                if "notifications" in metric.query_text.lower():
                    if (
                        "user_id" in metric.query_text.lower()
                        and "is_read" in metric.query_text.lower()
                    ):
                        recommendations.append(
                            OptimizationRecommendation(
                                component="database",
        ...
        try:
            ...
            for i in range(test_operations):
                ...
                await advanced_redis_client.set_with_layer(
                    test_key, json.dumps(test_value), layer, 300
                )
