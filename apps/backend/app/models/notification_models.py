...
class Notification(Base):
    ...
    created_at = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.timezone.utc))
    ...
    def is_expired(self) -> bool:
        ...
        if self.expires_at is None:
            return False
        return datetime.now(timezone.timezone.utc) > self.expires_at

    @property
    def age_seconds(self) -> int:
        """Get notification age in seconds"""
        return int((datetime.now(timezone.timezone.utc) - self.created_at).total_seconds())

    def mark_as_read(self) -> None:
        """Mark notification as read with timestamp"""
        if not self.is_read:
            self.is_read = True
            self.read_at = datetime.now(timezone.timezone.utc)

    def mark_as_delivered(self) -> None:
        """Mark notification as delivered with timestamp"""
        if not self.is_delivered:
            self.is_delivered = True
            self.delivered_at = datetime.now(timezone.timezone.utc)

    def mark_as_clicked(self) -> None:
        """Mark notification as clicked with timestamp"""
        self.clicked_at = datetime.now(timezone.timezone.utc)
        if not self.is_read:
            self.mark_as_read()
    ...
    def dismiss(self) -> None:
        ...
        self.dismissed_at = datetime.now(timezone.timezone.utc)
        if not self.is_read:
            self.mark_as_read()
...
class NotificationPreference(Base):
    ...
    # Metadata
    created_at = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.timezone.utc))
    updated_at = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.timezone.utc))
    ...
    def is_in_quiet_hours(self, check_time: datetime | None = None) -> bool:
        ...
        if check_time is None:
            check_time = datetime.now(timezone.timezone.utc)
