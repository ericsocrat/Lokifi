...
class TestCryptoDataService:
    ...
    async def test_set_cached_data(self, mock_redis):
        ...
        await service._set_cached("test_key", data, ttl=300)
    ...
    async def test_fetch_with_cache_cycle(self, mock_redis, sample_market_data):
        ...
        async with CryptoDataService() as service:
            with patch.object(service.client, 'get', return_value=mock_response) as mock_get:
                ...
                mock_redis.get = AsyncMock(return_value=json.dumps(sample_market_data))
...
class TestCryptoDataServiceEdgeCases:
    ...
    async def test_fetch_with_network_error(self, mock_redis):
        ...
        async with CryptoDataService() as service:
            with patch.object(service.client, 'get', side_effect=httpx.NetworkError("Connection failed")):
    ...
    async def test_fetch_with_timeout(self, mock_redis):
        ...
        async with CryptoDataService() as service:
            with patch.object(service.client, 'get', side_effect=httpx.TimeoutException("Request timed out")):
