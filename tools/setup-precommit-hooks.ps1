...
# Run enhanced protection with relaxed settings for pre-commit
if [ -f "`$TOOLS_DIR/enhanced-ci-protection.ps1" ]; then
    echo "🔍 Running enhanced protection checks..."
    
    if `$SHELL_CMD -ExecutionPolicy Bypass -File "`$TOOLS_DIR/enhanced-ci-protection.ps1" -CoverageThreshold $CoverageThreshold $(if ($Strict) { "-EnforceStrict" } else { "" }); then
        echo ""
    ...
    if [ -f "`$TOOLS_DIR/lokifi.ps1" ]; then
        if `$SHELL_CMD -ExecutionPolicy Bypass -File "`$TOOLS_DIR/lokifi.ps1" validate -Quick; then
...
if [ -f "`$TOOLS_DIR/enhanced-ci-protection.ps1" ]; then
    ...
    PUSH_COVERAGE=$((CoverageThreshold + 5))
    
    if `$SHELL_CMD -ExecutionPolicy Bypass -File "`$TOOLS_DIR/enhanced-ci-protection.ps1" -CoverageThreshold `$PUSH_COVERAGE -GenerateReport; then
        echo ""
        ...
        if [ -f "`$TOOLS_DIR/lokifi.ps1" ]; then
            if `$SHELL_CMD -ExecutionPolicy Bypass -File "`$TOOLS_DIR/lokifi.ps1" test -Quick; then
                echo "✅ Tests passed!"
            else
