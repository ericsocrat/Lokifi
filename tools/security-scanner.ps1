...
function Invoke-SecurityScan {
  <#
    ...
    .EXAMPLE
        Invoke-SecurityScan -Quick -CIMode
        # Fast scan with JSON output for CI/CD
    #>
  ...
  if (-not $CIMode) {
    ...
    if ($DryRun) {
      Write-Host ''
      Write-Host '🔍 DRY RUN MODE - Preview scan without executing fixes' -ForegroundColor Yellow
      Write-Host ''
    }
        ...
        if ($audit.metadata) {
          ...
          if ($results.vulnerabilities.total -eq 0) {
            ...
            if ($Fix) {
              if ($DryRun) {
                Write-Host ''
                Write-Host '� Would run: npm audit fix --force' -ForegroundColor Yellow
              } else {
                Write-Host ''
  ...
  if ($CIMode) {
    ...
    if ($results.vulnerabilities.critical -gt 0) {
      $errors += "$($results.vulnerabilities.critical) critical vulnerabilities found"
    }
    if ($results.vulnerabilities.high -gt 0) {
      $warnings += "$($results.vulnerabilities.high) high-severity vulnerabilities found"
    }
    if ($results.vulnerabilities.moderate -gt 0) {
      $warnings += "$($results.vulnerabilities.moderate) moderate-severity vulnerabilities found"
    }
    if ($score -lt 70) {
      $warnings += "Security score below threshold: $score/100"
    }

    $success = ($results.vulnerabilities.critical -eq 0) -and ($score -ge 50)
    ...
    # Exit codes: 0=success, 1=critical issues, 2=warnings
    if ($results.vulnerabilities.critical -gt 0) {
      exit 1
