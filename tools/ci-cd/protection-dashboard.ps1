...
function Get-TestCoverage {
    try {
        if (Get-Command Get-TestCoverageHeuristic -Module common -ErrorAction SilentlyContinue) {
            return (Get-TestCoverageHeuristic)
        }
    } catch { }
    ...
    $frontendTests = (Get-FileCount -Pattern "*.test.*" -Path (Join-Path $repoRoot "frontend")) + (Get-FileCount -Pattern "*.spec.*" -Path (Join-Path $repoRoot "frontend"))
    $frontendComponents = (Get-FileCount -Pattern "*.tsx" -Path (Join-Path $repoRoot "frontend/src")) + (Get-FileCount -Pattern "*.ts" -Path (Join-Path $repoRoot "frontend/src"))
    ...
    $overallCoverage = if ($totalSources -gt 0) { [math]::Round((($backendTests + $frontendTests) / $totalSources) * 100, 1) } else { 0 }
    return @{ Backend = @{ Coverage = $backendCoverage; Tests = $backendTests; Sources = $backendSources }; Frontend = @{ Coverage = $frontendCoverage; Tests = $frontendTests; Sources = $frontendComponents }; Overall = $overallCoverage }
}

function Get-SecurityStatus {
    try {
        if (Get-Command Get-SecurityStatusBasic -Module common -ErrorAction SilentlyContinue) {
            return (Get-SecurityStatusBasic)
        }
    } catch { }
    $securityIssues = @{
        Critical = 0
    ...
    try {
        $frontendPath = Join-Path (Get-RepoRoot) "frontend"
...
function Get-ProtectionStatus {
    ...
    $preCommitHooks = try { if (Get-Command Get-PreCommitHookStatus -Module common -ErrorAction SilentlyContinue) { Get-PreCommitHookStatus -RepoRoot $repoRoot } else { Test-Path (Join-Path $repoRoot ".git/hooks/pre-commit") } } catch { Test-Path (Join-Path $repoRoot ".git/hooks/pre-commit") }
    $ciWorkflows = try { if (Get-Command Get-WorkflowsCount -Module common -ErrorAction SilentlyContinue) { Get-WorkflowsCount -RepoRoot $repoRoot } else {
        $workflowsPath = Join-Path $repoRoot ".github/workflows"; $n=0; if (Test-Path $workflowsPath) { $n += (Get-ChildItem -Path $workflowsPath -Filter "*.yml" -File -ErrorAction SilentlyContinue).Count; $n += (Get-ChildItem -Path $workflowsPath -Filter "*.yaml" -File -ErrorAction SilentlyContinue).Count }; $n } } catch { 0 }
    $branchProtection = $false
    return @{ EnhancedProtection=$enhancedProtection; CoverageBooster=$coverageBooster; PreCommitHooks=$preCommitHooks; CIWorkflows=$ciWorkflows; BranchProtection=$branchProtection }
}
